...
pub const Path = struct {
    ...
    inline fn getPathLenT(comptime T: type, lenSum: usize) usize {
        const size = PathSize(T);
        if (lenSum > size.MIN) return @max(lenSum, size.MAX);
        return @max(lenSum, size.MIN);
    }
    ...
    pub fn PathSize(comptime T: type) type {
        // u8 path MIN and MAX are the same because bun.MAX_PATH_BYTES is already small.
        const _MIN: usize = if (T == u16) PATH_MIN_WIDE else bun.MAX_PATH_BYTES;
        const _MAX: usize = if (T == u16) windows.PATH_MAX_WIDE else bun.MAX_PATH_BYTES;
        return struct {
            const MIN = _MIN;
            const MAX = _MAX;
        };
    ...
    inline fn basenamePosix(globalObject: *JSC.JSGlobalObject, path: []const u8, suffix: ?[]const u8) JSC.JSValue {
        return toJSString(globalObject, basenamePosixT(u8, path, suffix));
    }

    inline fn basenameWindows(globalObject: *JSC.JSGlobalObject, path: []const u8, suffix: ?[]const u8) JSC.JSValue {
        return toJSString(globalObject, basenameWindowsT(u8, path, suffix));
    }
    ...
    pub fn basename(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        if (suffix_ptr) |_suffix_ptr| {
            ...
            const suffixLen = suffixZStr.len;
            if (suffixLen > 0 and suffixLen <= len) {
                var suffixZSlice = suffixZStr.toSlice(allocator);
                defer suffixZSlice.deinit();
                const suffix = suffixZSlice.slice();
                return if (isWindows) basenameWindows(globalObject, path, suffix) else basenamePosix(globalObject, path, suffix);
            }
        }
        return if (isWindows) basenameWindows(globalObject, path, null) else basenamePosix(globalObject, path, null);
    }
    ...
    inline fn dirnamePosix(globalObject: *JSC.JSGlobalObject, path: []const u8) JSC.JSValue {
        return toJSString(globalObject, dirnamePosixT(u8, path));
    }

    inline fn dirnameWindows(globalObject: *JSC.JSGlobalObject, path: []const u8) JSC.JSValue {
        return toJSString(globalObject, dirnameWindowsT(u8, path));
    }
    ...
    pub fn dirname(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        const pathZSlice = pathZStr.toSlice(allocator);
        defer pathZSlice.deinit();
        return if (isWindows) dirnameWindows(globalObject, pathZSlice.slice()) else dirnamePosix(globalObject, pathZSlice.slice());
    }
    ...
    inline fn extnamePosix(globalObject: *JSC.JSGlobalObject, path: []const u8) JSC.JSValue {
        return toJSString(globalObject, extnamePosixT(u8, path));
    }

    inline fn extnameWindows(globalObject: *JSC.JSGlobalObject, path: []const u8) JSC.JSValue {
        return toJSString(globalObject, extnameWindowsT(u8, path));
    }
    ...
    pub fn extname(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        const pathZSlice = pathZStr.toSlice(allocator);
        defer pathZSlice.deinit();
        return if (isWindows) extnameWindows(globalObject, pathZSlice.slice()) else extnamePosix(globalObject, pathZSlice.slice());
    }
    ...
    pub fn format(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        const size = comptime PathSize(u8);
        return switch (getPathLenT(u8, lenSum)) {
            size.MAX => if (isWindows) formatWindowsJS_L(u8, globalObject, pathObject, bun.MAX_PATH_BYTES) else formatPosixJS_L(u8, globalObject, pathObject, size.MAX),
            else => |bufLen| brk: {
                const buf = allocator.alloc(u8, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
                break :brk if (isWindows) formatWindowsJS_T(u8, globalObject, pathObject, buf) else formatPosixJS_T(u8, globalObject, pathObject, buf);
            },
        };
    }
    ...
    pub fn join(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        const size = comptime PathSize(u8);
        return switch (getPathLenT(u8, lenSum)) {
            size.MAX => if (isWindows) joinWindowsJS_L(u8, globalObject, paths, bun.MAX_PATH_BYTES) else joinPosixJS_L(u8, globalObject, paths, size.MAX),
            else => |bufLen| brk: {
                const buf = allocator.alloc(u8, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
                const buf2 = allocator.alloc(u8, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf2);
                break :brk if (isWindows) joinWindowsJS_T(u8, globalObject, paths, buf, buf2) else joinPosixJS_T(u8, globalObject, paths, buf, buf2);
            },
        };
    }
    ...
    inline fn normalizePosixJS_T(comptime T: type, globalObject: *JSC.JSGlobalObject, path: []const T, buf: []T) JSC.JSValue {
        return toJSString(globalObject, normalizePosixT(T, path, buf));
    }

    inline fn normalizeWindowsJS_T(comptime T: type, globalObject: *JSC.JSGlobalObject, path: []const T, buf: []T) JSC.JSValue {
        return toJSString(globalObject, normalizeWindowsT(T, path, buf));
    }

    inline fn normalizePosixJS_L(comptime T: type, globalObject: *JSC.JSGlobalObject, path: []const T, comptime len: usize) JSC.JSValue {
        var buf: [len]T = undefined;
        return normalizePosixJS_T(T, globalObject, path, &buf);
    }

    inline fn normalizeWindowsJS_L(comptime T: type, globalObject: *JSC.JSGlobalObject, path: []const T, comptime len: usize) JSC.JSValue {
        var buf: [len]T = undefined;
    ...
    pub fn normalize(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        const path = pathZSlice.slice();
        const size = comptime PathSize(u8);
        return switch (getPathLenT(u8, len)) {
            size.MAX => if (isWindows) normalizeWindowsJS_L(u8, globalObject, path, bun.MAX_PATH_BYTES) else normalizePosixJS_L(u8, globalObject, path, size.MAX),
            else => |bufLen| brk: {
                const buf = allocator.alloc(u8, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
                break :brk if (isWindows) normalizeWindowsJS_T(u8, globalObject, path, buf) else normalizePosixJS_T(u8, globalObject, path, buf);
            },
        };
    }
    ...
    inline fn parsePosix(globalObject: *JSC.JSGlobalObject, path: []const u8) JSC.JSValue {
        return parsePosixT(u8, path).toJSObject(globalObject);
    }

    inline fn parseWindows(globalObject: *JSC.JSGlobalObject, path: []const u8) JSC.JSValue {
        return parseWindowsT(u8, path).toJSObject(globalObject);
    }
    ...
    pub fn parse(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        const pathZSlice = pathZStr.toSlice(allocator);
        defer pathZSlice.deinit();
        return if (isWindows) parseWindows(globalObject, pathZSlice.slice()) else parsePosix(globalObject, pathZSlice.slice());
    }
    ...
    pub fn relative(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        const size = comptime PathSize(u8);
        return switch (getPathLenT(u8, lenSum)) {
            size.MAX => if (isWindows) relativeWindowsJS_L(u8, globalObject, from, to, bun.MAX_PATH_BYTES) else relativePosixJS_L(u8, globalObject, from, to, size.MAX),
            else => |bufLen| brk: {
                const buf = allocator.alloc(u8, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
                const buf2 = allocator.alloc(u8, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf2);
                const buf3 = allocator.alloc(u8, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf3);
                break :brk if (isWindows) relativeWindowsJS_T(u8, globalObject, from, to, buf, buf2, buf3) else relativePosixJS_T(u8, globalObject, from, to, buf, buf2, buf3);
            },
        };
    }
    ...
    pub fn resolve(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        const size = comptime PathSize(u8);
        return switch (getPathLenT(u8, lenSum)) {
            size.MAX => if (isWindows) resolveWindowsJS_L(u8, globalObject, paths, bun.MAX_PATH_BYTES) else resolvePosixJS_L(u8, globalObject, paths, size.MAX),
            else => |bufLen| brk: {
                const buf = allocator.alloc(u8, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
                const buf2 = allocator.alloc(u8, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf2);
                break :brk if (isWindows) resolveWindowsJS_T(u8, globalObject, paths, buf, buf2) else resolvePosixJS_T(u8, globalObject, paths, buf, buf2);
            },
        };
    }
    ...
    pub fn toNamespacedPath(globalObject: *JSC.JSGlobalObject, isWindows: bool, args_ptr: [*]JSC.JSValue, args_len: u16) callconv(.C) JSC.JSValue {
        ...
        const size = comptime PathSize(u8);
        return switch (getPathLenT(u8, len)) {
            size.MAX => toNamespacedPathWindowsJS_L(u8, globalObject, pathZSlice.slice(), size.MAX),
            else => |bufLen| brk: {
                const buf = allocator.alloc(u8, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf);
                const buf2 = allocator.alloc(u8, bufLen) catch bun.outOfMemory();
                defer allocator.free(buf2);
                break :brk toNamespacedPathWindowsJS_T(u8, globalObject, pathZSlice.slice(), buf, buf2);
            },
        };
    }
